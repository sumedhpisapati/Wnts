...
def a_star_route(start_lat, start_lon, target_lat, target_lon, res=9, profile=None, weather=None, initial_heading=0):
    ...
    heapq.heappush(queue, (0, counter, start_state, initial_heading))
    
    came_from = {}
    g_score = {start_state: 0.0}
    # Use a heuristic multiplier of 0.03 to accurately estimate remaining cost 
    # (since base cell cost is 5.0 and step is ~170m)
    f_score = {start_state: haversine_m(start_lat, start_lon, target_lat, target_lon) * 0.03}
    ...
    while queue:
        ...
        _, _, current_cell, current_heading = heapq.heappop(queue)
        
        if current_cell == target_cell:
            best_target_state = current_cell
            break
        ...
        for neighbor in get_h3_neighbors(current_cell):
            ...
            tentative_g = g_score[current_cell] + (step_dist * COST_WEIGHTS["distance"]) + cell_penalty + turn_penalty
            
            neighbor_state = neighbor
            
            if neighbor_state not in g_score or tentative_g < g_score[neighbor_state]:
                came_from[neighbor_state] = current_cell
                g_score[neighbor_state] = tentative_g
                
                h = haversine_m(n_lat, n_lon, t_lat, t_lon) * 0.03
                f = tentative_g + h
                
                counter += 1
                heapq.heappush(queue, (f, counter, neighbor_state, bearing_deg))
    ...
    if current_state is None or current_state not in came_from:
        # closest state
        if g_score:
            closest_state = min(g_score.keys(), key=lambda s: haversine_m(h3.cell_to_latlng(s)[0], h3.cell_to_latlng(s)[1], t_lat, t_lon))
