...
def score_with_time(props, dt, curves, weather):
    ...
    outdoor={"road","water","forest","commercial"}
    wx=weather.get("outdoor_factor",1.0) if lu in outdoor else 1.0
    ...
    expl=(f"At {dt.strftime('%H:%M')} on a {day}, {int(pres*100)}% of peak population "
          ...
          f"(outdoor factor {wx:.2f}). Static TEL={base:.0f} → {tel_t:.0f}. "
...
def compute_debris_landing(lat, lon, alt_m, weather):
    """
    For each debris class, compute:
      - Time of flight (alt / terminal_velocity)
      - Horizontal drift (wind_speed * tof * Cd_h)
      - Landing point (lat + drift in wind direction)
      - Scatter radius (uncertainty = 20% of drift + min 50m)

    Returns list of landing zones per debris class.

    NOT heuristic: equations are Newtonian mechanics.
    Parameters (mass, Cd, area) are estimated from literature.
    """
    wind_kmh  = weather.get("windspeed_kmh", 10)
    wind_ms   = wind_kmh / 3.6
    # Wind direction: meteorological convention (direction FROM which wind blows)
    # We need direction TO which debris drifts = wind_from + 180
    wind_from = weather.get("wind_direction_deg", 270)  # default W wind
    wind_to   = math.radians((wind_from + 180) % 360)
    # Convert to geographic bearing: 0=N, 90=E
    # wind_to is in standard math angle (0=E, 90=N), convert:
    # geographic_bearing = 90 - math_angle
    bear_rad  = math.radians(90) - wind_to  # bearing in radians from N

    landing_zones = []
    for dc in DEBRIS_CLASSES:
        v_t  = terminal_velocity(dc["mass_kg"], dc["Cd_v"], dc["area_m2"])
        tof  = alt_m / v_t                          # seconds to fall
        # Horizontal drift: wind carries fragment, reduced by horizontal drag
        # drift = v_wind * tof * (1 / (1 + Cd_h))  [simplified momentum model]
        drift_m = wind_ms * tof * (1.0 / (1.0 + dc["Cd_h"]))

        # Convert drift to lat/lon offset
        # North component: drift * cos(bearing)
        # East component:  drift * sin(bearing)
        d_lat = (drift_m * math.cos(bear_rad)) / 111111.0
        d_lon = (drift_m * math.sin(bear_rad)) / (111111.0 * math.cos(math.radians(lat)))

        land_lat = lat + d_lat
        land_lon = lon + d_lon

        # Scatter radius: uncertainty in landing point
        # 25% of drift (wind variability) + 50m minimum (intercept accuracy)
        scatter_m = max(50, drift_m * 0.25)
...
def score_landing_zones(landing_zones):
    """
    Sample grid cells within each debris class scatter radius.
    Weighted by debris class consequence weight and cell unsuitability.

    Returns:
      - total_consequence: float 0-1 (weighted score)
      - forbidden_hit: bool
      - pop_at_risk: int
      - details: per-class breakdown
    """
    ...
    for zone in landing_zones:
        ...
        for feat in GRID_FEATURES:
            ...
            if dist <= rad:
                cells_in_zone.append(p)
                if p.get("is_forbidden"): forbidden_hit = True
                pop_at_risk += p.get("pop_count", 0)

        if not cells_in_zone:
            S,N,W,E = BBOX
            if S<=zlat<=N and W<=zlon<=E:
                zone_score = 0.05
            else:
                zone_score = 0.0
        else:
            # Blend direct cell score + integrated_risk (1km neighbourhood score)
            # 70% direct + 30% area consequence — captures nearby population clusters
            # integrated_risk normalised from ~0-50 range to 0-1
            direct = sum(c.get("score") or 0 for c in cells_in_zone) / len(cells_in_zone)
            integ  = sum(min(c.get("integrated_risk",0)/20.0,1.0) for c in cells_in_zone) / len(cells_in_zone)
            zone_score = 0.70 * direct + 0.30 * integ
        ...
        details.append({
            ...
            "cells_hit":   len(cells_in_zone),
            "forbidden":   any(c.get("is_forbidden") for c in cells_in_zone),
            "pop_in_zone": sum(c.get("pop_count",0) for c in cells_in_zone),
        })
...
def generate_drone_trajectory(seed, target_key=None):
    ...
    All waypoints constrained to BBOX so every point is scoreable.

    Entry points: edges of the BBOX (sea side — south/east/west)
    Intermediate waypoints: clamped inside BBOX with margin
    Patterns: s_curve, flanking, sea_skim, pincer, direct_jink, spiral_in
    Speed: 80km/h = 22.2 m/s
    """
    import random
    rng = random.Random(seed)

    S, N, W, E = BBOX   # 56.10, 56.25, 15.45, 15.75

    # Entry points: on the BBOX boundary edges (sea approaches)
    # All inside or on the edge of the scored region
    SEA_ENTRIES = [
    ...
    HIGH_VALUE_TARGETS = [
        ...
        {"key":"stumholmen","name":"Stumholmen",            "lat":56.1590,"lon":15.5920,"value":0.65},
    ]

    entry  = rng.choice(SEA_ENTRIES)
    target = rng.choices(HIGH_VALUE_TARGETS, weights=[t["value"] for t in HIGH_VALUE_TARGETS])[0]
    pattern = rng.choice(["s_curve","flanking","sea_skim","pincer","direct_jink","spiral_in"])

    # Clamp lat/lon to stay inside BBOX with a small margin
    margin_lat = (N - S) * 0.04
    margin_lon = (E - W) * 0.04
    def clamp(lat, lon):
        return (
            max(S + margin_lat, min(N - margin_lat, lat)),
            max(W + margin_lon, min(E - margin_lon, lon))
        )

    def mk(lat, lon, alt, label):
        lat, lon = clamp(lat, lon)
        return {"lat":round(lat,6),"lon":round(lon,6),"alt_m":int(alt),"label":label,"t":0}

    def midpoint_offset(frac, perp_m):
        """Point at frac along entry→target, offset perp_m perpendicular, clamped."""
        mlat = entry["lat"] + frac*(target["lat"]-entry["lat"])
        mlon = entry["lon"] + frac*(target["lon"]-entry["lon"])
        bear = math.atan2(target["lon"]-entry["lon"], target["lat"]-entry["lat"])
        perp = bear + math.pi/2
        sign = rng.choice([-1,1])
        dlat = sign*perp_m*math.cos(perp)/111111.0
        dlon = sign*perp_m*math.sin(perp)/(111111.0*math.cos(math.radians(mlat)))
        return clamp(mlat+dlat, mlon+dlon)
    ...
    # ── Pattern generation ────────────────────────────────────────────────────
    if pattern == "s_curve":
        la1,lo1 = midpoint_offset(0.28, rng.uniform(3000,6000))
        la2,lo2 = midpoint_offset(0.65, rng.uniform(2500,5000))
        # Force second point to opposite side
        la2b = tlat + (elat-tlat)*0.65
        lo2b = tlon + (elon-tlon)*0.65
        la2,lo2 = midpoint_offset(0.65, rng.uniform(2500,5000))
        waypoints = [
            mk(elat,elon,260,"Entry: "+entry["label"]),
        ...
        la,lo = midpoint_offset(0.55, rng.uniform(4000,7000))
        la2,lo2 = midpoint_offset(0.72, rng.uniform(2000,4000))
        waypoints = [
            mk(elat,elon,280,"Entry: "+entry["label"]),
        ...
        for i in range(n_pts):
            frac = (i+1)/(n_pts+1)
            clat = elat + frac*(tlat-elat) + rng.uniform(-0.025,0.025)
            clon = elon + frac*(tlon-elon) + rng.uniform(-0.025,0.025)
            coast_pts.append(mk(clat,clon,rng.uniform(25,70),f"Skim {i+1}"))
        waypoints = [
            mk(elat,elon,70,"Entry: "+entry["label"]+" (low)"),
            *coast_pts,
            mk(tlat,tlon,50,"Target: "+target["name"]),
        ]

    elif pattern == "pincer":
        pivot_lat = elat+(tlat-elat)*0.50 + rng.uniform(-0.04,-0.01)
        pivot_lon = elon+(tlon-elon)*0.50 + rng.uniform(-0.03,0.03)
        waypoints = [
            mk(elat,elon,280,"Entry: "+entry["label"]),
            mk(elat+(pivot_lat-elat)*0.5, elon+(pivot_lon-elon)*0.5, rng.uniform(230,290),"Leg 1 mid"),
            mk(pivot_lat,pivot_lon,rng.uniform(160,240),"Pivot"),
            mk(pivot_lat+(tlat-pivot_lat)*0.45, pivot_lon+(tlon-pivot_lon)*0.45, rng.uniform(100,170),"Leg 2"),
            mk(tlat,tlon,50,"Target: "+target["name"]),
        ]

    elif pattern == "direct_jink":
        n_jinks = rng.randint(3,5)
        jink_pts = []
        for i in range(n_jinks):
            frac = (i+1)/(n_jinks+1)
            jlat = elat + frac*(tlat-elat) + rng.uniform(-0.018,0.018)
            jlon = elon + frac*(tlon-elon) + rng.uniform(-0.022,0.022)
            alt  = rng.uniform(90,260) if frac < 0.7 else rng.uniform(55,130)
            jink_pts.append(mk(jlat,jlon,alt,f"Jink {i+1}"))
        waypoints = [
            mk(elat,elon,240,"Entry: "+entry["label"]),
            *jink_pts,
            mk(tlat,tlon,50,"Target: "+target["name"]),
        ]

    else:  # spiral_in — tightened radius so it stays in bbox
        n_pts = rng.randint(4,6)
        rad_start = rng.uniform(0.025,0.045)  # much tighter than before
        spiral_pts = []
        for i in range(n_pts):
            angle  = (i/n_pts)*2*math.pi + rng.uniform(0,0.4)
            radius = rad_start*(1 - i/n_pts*0.65)
            clat   = tlat + radius*math.sin(angle)
            clon   = tlon + radius*math.cos(angle)/math.cos(math.radians(tlat))
            alt    = 230 - i*(170/n_pts)
            spiral_pts.append(mk(clat,clon,max(alt,55),f"Spiral {i+1}"))
        waypoints = [
            mk(elat,elon,270,"Entry: "+entry["label"]),
            mk(elat+(tlat-elat)*0.45+rng.uniform(-0.02,0.02),
               elon+(tlon-elon)*0.45+rng.uniform(-0.02,0.02), 200,"Transit"),
            *spiral_pts,
            mk(tlat,tlon,40,"Target: "+target["name"]),
        ]
    ...
    return {
        ...
        "to_lat":       target["lat"],
        "to_lon":       target["lon"],
        "description":  (f"[{pattern.upper().replace('_',' ')}] "
                         f"{entry['label']} → {target['name']} | "
                         f"{total_dist/1000:.1f}km | {total_dist/speed_ms/60:.1f}min"),
        "assumptions":  "Speed=80km/h, all waypoints within scored BBOX, Hoerner 1965 Cd",
    }
...
def solve_intercept_feasibility(drone_lat, drone_lon, drone_time_s,
    ...
    return {
        ...
        "time_ok":           time_ok,
        # Expose for UI
        "dist_m":            round(dist_now),   # backwards compat
    }
...
def full_engagement_analysis(traj, system_key, interceptor_pos, wx):
    ...
    if total_d > 0:
        ...
        drone_vlat = drone_vlon = 0.0

    for (t_param, t_sec, lat, lon, alt_m) in timed:

        # ── Interceptor feasibility (lead-angle geometry) ──────────────────
        ...
        if S <= lat <= N and W <= lon <= E:
            ...
            # Outside scored region — assume open sea if no land features
            consequence   = 0.05
            forbidden_hit = False
            pop_at_risk   = 0
            details       = []
        ...
        #
        # PRIORITY ORDER:
        #   NEVER   — inside exclusion zone OR forbidden infrastructure hit
        #   NO SHOT — interceptor can't reach (too far, too fast, too close)
        #   HOLD    — feasible but consequence too high
        #   CAUTION — feasible, marginal consequence
        #   ENGAGE  — feasible, low consequence, before exclusion zone
        #
        # Optimal = ENGAGE with lowest consequence,
        #           tiebreak: latest t (maximise decision time)

        if inside_excl:
            decision = "NEVER"
            reason   = f"Inside exclusion zone: {inside_site['name']}"
        elif forbidden_hit:
            decision = "NEVER"
            reason   = "Debris hits forbidden infrastructure"
        ...
        elif not before_excl:
            decision = "NEVER"
            reason   = f"Drone already past exclusion zone at t={earliest_excl:.0f}s"
        elif consequence > 0.55:
            decision = "HOLD"
            reason   = f"Consequence too high ({consequence:.2f})"
        elif consequence > 0.30:
            decision = "CAUTION"
            reason   = f"Marginal consequence ({consequence:.2f}) — seek better window"
        else:
            decision = "ENGAGE"
            reason   = f"Consequence acceptable ({consequence:.2f}), interceptor feasible"
    ...
    # ENGAGE candidates only
    engage_pts = [c for c in candidates if c["decision"] == "ENGAGE"]
    caution_pts= [c for c in candidates if c["decision"] == "CAUTION"]

    if engage_pts:
        # Lowest consequence; tiebreak: latest t (max decision time)
        min_con = min(c["consequence"] for c in engage_pts)
        # All points within 20% of minimum consequence
        good = [c for c in engage_pts if c["consequence"] <= min_con * 1.25]
        # Latest of those
        optimal = max(good, key=lambda c: c["t"])
        optimal_type = "ENGAGE"
    elif caution_pts:
        optimal = min(caution_pts, key=lambda c: c["consequence"])
        optimal_type = "CAUTION — no clean window found"
    else:
        # No feasible low-consequence point — pick least-bad
        valid = [c for c in candidates
                 if c["feasibility"]["feasible"] and c["before_excl"] and not c["inside_excl"]]
        if valid:
            optimal = min(valid, key=lambda c: c["consequence"])
            optimal_type = "NO CLEAN WINDOW — least-bad option"
        else:
    ...
    def do_GET(self):
        ...
        if path=="/score":
            ...
            if "time" in qs or "day" in qs:
                dt=parse_dt(qs); wx=fetch_weather()
            ...
            wx = fetch_weather()
            wx["wind_direction_deg"] = float(qs.get("wind_dir",["270"])[0])

            # Interceptor position: default naval base, overrideable
            # Always use naval base as interceptor position
            # (can be overridden via ?iLat=&iLon= for future multi-site support)
            i_lat = float(qs.get("iLat",[str(DEFAULT_INTERCEPTOR["lat"])])[0])
            i_lon = float(qs.get("iLon",[str(DEFAULT_INTERCEPTOR["lon"])])[0])
            interceptor_pos = {"lat":i_lat,"lon":i_lon}
            print(f"[INTERCEPT] Site: {i_lat:.4f}N {i_lon:.4f}E | System: {system_key}")
...
<div id="hud">
  ...
  <div id="wx-bar">
    <div class="wx-item"><div class="wx-v" id="wx-temp">—°C</div><div class="wx-l">TEMP</div></div>
    <div class="wx-item"><div class="wx-v" id="wx-rain">—mm</div><div class="wx-l">RAIN</div></div>
    <div class="wx-item"><div class="wx-v" id="wx-wind">—km/h</div><div class="wx-l">WIND</div></div>
    <div id="wx-cond">Loading…</div>
    <div id="wx-factor"></div>
  </div>
  <div id="time-ctrl">
    <div class="tc-row">
...
function timeParams(){
  const {h,dt}=hday();
  return `&time=${String(h).padStart(2,'0')}:00&day=${dt}`;
}
function tempTEL(base,lu,h,dt){
  if(base>=9999) return 9999;
  const outdoor=['road','water','forest','commercial'];
  const wx=outdoor.includes(lu)?WX_FACTOR:1.0;
  return base*pres(lu,h,dt)*wx;
}
...
const DCISION_COLS={"ENGAGE":"#00ff88","CAUTION":"#ffc107","HOLD":"#ff7043","NO SHOT":"#446688","NEVER":"#aa00aa"};
...
function buildTimeline(cands){
  ...
  const legend=Object.entries(DCISION_COLS).map(([d,c],i)=>`<rect x="${pad+i*48}" y="${H-13}" width="8" height="6" fill="${c}"/><text x="${pad+i*48+10}" y="${H-7}" fill="${c}" font-size="6">${d}</text>`).join('');
...
function renderFrame(t){
  ...
  if(intercepted){
    // ── INTERCEPTED: show explosion, stop drone ────────────────────────
    // Explosion icon at intercept point
    const explIcon=L.divIcon({
      html:`<div style="font-size:32px;line-height:32px;animation:none;filter:drop-shadow(0 0 8px #ff4400)">💥</div>`,
      className:'',iconSize:[36,36],iconAnchor:[18,18]
    });
    const ex=L.marker([c.lat,c.lon],{icon:explIcon})
      .bindTooltip(`<div style="font-family:monospace;font-size:11px;line-height:1.5">
        <b>💥 INTERCEPTED</b><br>t=${c.time_s?.toFixed(0)}s | Alt:${c.alt_m}m<br>
        Consequence:${(c.consequence*100).toFixed(0)}%<br>${c.reason}</div>`,
        {permanent:false,opacity:0.95});
    ex._sf=true;ex.addTo(simLayer);

    // Straight solid line from interceptor to intercept point
    if(interceptorPos){
      const missLine=L.polyline(
        [[interceptorPos.lat,interceptorPos.lon],[c.lat,c.lon]],
        {color:'#00ccff',weight:3,opacity:1.0}   // solid, not dashed
      );
    ...
    if(panel)panel.innerHTML=`
      <div style="display:flex;justify-content:space-between;align-items:baseline">
        <span style="color:#666;font-size:8px">INTERCEPTED t=${c.time_s?.toFixed(0)}s alt=${c.alt_m}m</span>
        <span style="font-size:16px;font-weight:bold;color:#00ff88">NEUTRALISED</span>
      </div>
      <div style="color:#00ff88;font-size:9px;font-weight:bold">DRONE DESTROYED — MISSION COMPLETE</div>
      <div style="color:#888;font-size:8px">Consequence:${(c.consequence*100).toFixed(0)}% | ${c.reason}</div>
      ${c.pop_at_risk>0?`<div style="color:#f88;font-size:8px">&#9888; ${c.pop_at_risk} civilians at risk from debris</div>`:''}`;
    updateCursor();
    return;
  }
  ...
  // ── Interceptor missile line ───────────────────────────────────────────
  // Straight solid line from interceptor to drone current position.
  // Only shown when ENGAGE decision (missile is actually fired).
  if(interceptorPos && c.decision==='ENGAGE'){
    const missLine=L.polyline(
      [[interceptorPos.lat,interceptorPos.lon],[c.lat,c.lon]],
      {color:'#00ccff',weight:2.5,opacity:0.9}   // solid straight line
    );
    missLine._sf=true;missLine.addTo(simLayer);
    // Small missile dot moving along the line
    const midLat=(interceptorPos.lat+c.lat)/2;
    const midLon=(interceptorPos.lon+c.lon)/2;
    const missHead=Math.atan2(c.lon-interceptorPos.lon,c.lat-interceptorPos.lat)*180/Math.PI;
    const missIcon=L.divIcon({
      html:`<div style="color:#00ccff;font-size:12px;transform:rotate(${missHead}deg)">➤</div>`,
      className:'',iconSize:[14,14],iconAnchor:[7,7]
    });
    const mm=L.marker([midLat,midLon],{icon:missIcon});
    mm._sf=true;mm.addTo(simLayer);
  }

  // Debris landing zones
  (c.landing_zones||[]).forEach(z=>{
    const col=DCOLS[z.class]||'#ff9900';
    const ci=L.circle([z.land_lat,z.land_lon],{radius:z.scatter_m,color:col,fillColor:col,fillOpacity:0.12,weight:1.5})
      .bindTooltip(`<div style="font-family:monospace;font-size:10px"><b>${z.class}</b><br>Drift:${z.drift_m}m ToF:${z.tof_s}s Scatter:±${z.scatter_m}m</div>`,{sticky:true});
    ci._sf=true;ci.addTo(simLayer);
    const ln=L.polyline([[c.lat,c.lon],[z.land_lat,z.land_lon]],{color:col,weight:1,dashArray:'3,3',opacity:0.4});
    ln._sf=true;ln.addTo(simLayer);
  });
...
async function runAnalysis(){
  ...
  try{
    ...
    const url=`/intercept?seed=${simDrone.seed}&system=${sys}&iLat=${iPos2.lat}&iLon=${iPos2.lon}&wind_dir=${wx.wind_direction_deg||270}`;
    ...
    // Optimal debris zones
    (opt.landing_zones||[]).forEach(z=>{
      const col=DCOLS[z.class]||'#ff9900';
      L.circle([z.land_lat,z.land_lon],{radius:z.scatter_m,color:col,fillColor:col,fillOpacity:0.18,weight:2})
       .bindTooltip(`<div style="font-family:monospace;font-size:10px"><b>${z.class}</b><br>Drift:${z.drift_m}m ToF:${z.tof_s}s</div>`,{sticky:true})
       .addTo(simLayer);
      L.polyline([[opt.lat,opt.lon],[z.land_lat,z.land_lon]],{color:col,weight:1.5,dashArray:'4,3',opacity:0.5}).addTo(simLayer);
    });
    ...
    if(ir2)ir2.innerHTML=
      ...
      `<span style="color:#00c8a0">✓${st.engage_pts||0} ⚠${st.caution_pts||0} ✗${st.no_shot_pts||0} ⊘${st.never_pts||0}</span><br>`+
